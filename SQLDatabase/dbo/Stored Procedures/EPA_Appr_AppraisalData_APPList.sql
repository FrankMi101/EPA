...
CREATE proc [dbo].[EPA_Appr_AppraisalData_APPList]    -- '0','mif','20172018','0528','00010452','Session1','PPA' 
	@Operate		varchar(30),
	@UserID			varchar(30),
...
if @Operate ='CheckAPP'
   begin
		if exists (select * from  [dbo].[EPA_Appr_Comments_APP] where School_Year =@SchoolYear and School_Code = @SchoolCode and SessionID = @SessionID and EmployeeID = @EmployeeID and ItemCode = @ItemCode )
		   select 'View Last APP'
        else
			select 'RollOver Last APP'
   end  
   ...
   begin
		...
		insert into  [dbo].[EPA_Appr_Comments_APP] 
		...
		from dbo.EPA_Appr_Comments_APP
		...
		select   SequenceNo, [AreaForGrowth],[GrowthStrategy],[TimeLine],[FutureGrowth]
		from  [dbo].[EPA_Appr_Comments_APP] 
   ...
   */
if @Operate ='AddNewGoal'
    begin
		declare @GoalCount int
		set @GoalCount = (select top 1 SequenceNo from [dbo].[EPA_Appr_Comments_APP] where School_Year =@SchoolYear and School_Code = @SchoolCode and SessionID = @SessionID and EmployeeID =@EmployeeID
						  order by SequenceNo DESC)
        insert into  [dbo].[EPA_Appr_Comments_APP] 
			([School_Year],[School_Code],[SessionID],[EmployeeID],[ItemCode],[SequenceNo],[Goals],[StrategyAction],[Practices],[Indicators],[Results])
			values (@SchoolYear,@SchoolCode,@SessionID,@EmployeeID,@ItemCode,@GoalCount + 1 ,'','','','','')

		select   SequenceNo as SeqNo, [Goals],[StrategyAction],[Practices],[Indicators],[Results]
		from  [dbo].[EPA_Appr_Comments_APP] 
		where School_Year =@SchoolYear and School_Code = @SchoolCode and SessionID = @SessionID and EmployeeID =@EmployeeID and ItemCode = @ItemCode  
		order by SeqNo
	end
	...
	begin
		begin try
			if exists (select * from  [dbo].[EPA_Appr_Comments_APP] where School_Year =@SchoolYear and School_Code = @SchoolCode and SessionID = @SessionID and EmployeeID =@EmployeeID and ItemCode = @ItemCode and SequenceNo = @SeqNo)	
				begin
				    insert into dbo.EPA_Appr_Comments_APP_Bak
					...
					delete[dbo].[EPA_Appr_Comments_APP]
					...
					set @cInt =1
					  ...
					  WHILE  @@FETCH_STATUS = 0
						  begin
							 if @Seq != @cInt
							    update [dbo].[EPA_Appr_Comments_APP]  set SequenceNo = @cInt
    ...
    begin
		if not exists (select * from  [dbo].[EPA_Appr_Comments_APP] where School_Year =@SchoolYear and School_Code = @SchoolCode and SessionID = @SessionID and EmployeeID =@EmployeeID )
			insert into  [dbo].[EPA_Appr_Comments_APP] 
			([School_Year],[School_Code],[SessionID],[EmployeeID],[ItemCode],[SequenceNo],[Goals],[StrategyAction],[Practices],[Indicators],[Results])
		...
		begin try
			if exists (select * from  [dbo].[EPA_Appr_Comments_APP] where School_Year =@SchoolYear and School_Code = @SchoolCode and SessionID = @SessionID and EmployeeID =@EmployeeID and ItemCode = @ItemCode and SequenceNo = @SeqNo)	
				begin
					...
					if 	@ActionItem ='Goals'	     
						  update  [dbo].[EPA_Appr_Comments_APP]
						  set Goals = @Value, lu_date =getdate(), lu_user = @UserID, lu_function = app_name()
						  where School_Year =@SchoolYear and School_Code = @SchoolCode and SessionID = @SessionID and EmployeeID =@EmployeeID and SequenceNo = @SeqNo --  and ItemCode = @ItemCode
 		 
					else if @ActionItem ='StrategyAction'	     
						  update  [dbo].[EPA_Appr_Comments_APP]
						  set StrategyAction = @Value, lu_date =getdate(), lu_user = @UserID, lu_function = app_name()
						  where School_Year =@SchoolYear and School_Code = @SchoolCode and SessionID = @SessionID and EmployeeID =@EmployeeID and SequenceNo = @SeqNo --  and ItemCode = @ItemCode
					else if @ActionItem ='Practices'	     
						  update  [dbo].[EPA_Appr_Comments_APP]
						  set Practices = @Value, lu_date =getdate(), lu_user = @UserID, lu_function = app_name()
						  where School_Year =@SchoolYear and School_Code = @SchoolCode and SessionID = @SessionID and EmployeeID =@EmployeeID and SequenceNo = @SeqNo --  and ItemCode = @ItemCode
					else if @ActionItem ='Indicators'	     
						  update  [EPA_Appr_Comments_APP]
						  set Indicators = @Value, lu_date =getdate(), lu_user = @UserID, lu_function = app_name()
						  where School_Year =@SchoolYear and School_Code = @SchoolCode and SessionID = @SessionID and EmployeeID =@EmployeeID and SequenceNo = @SeqNo --  and ItemCode = @ItemCode
					else if @ActionItem ='Results'	     
						  update  [EPA_Appr_Comments_APP]
						  set Results = @Value, lu_date =getdate(), lu_user = @UserID, lu_function = app_name()
						  where School_Year =@SchoolYear and School_Code = @SchoolCode and SessionID = @SessionID and EmployeeID =@EmployeeID and SequenceNo = @SeqNo --  and ItemCode = @ItemCode
				end
			Select 'Successfully' as rValue
       end try

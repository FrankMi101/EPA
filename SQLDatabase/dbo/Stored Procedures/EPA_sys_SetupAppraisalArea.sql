...
CREATE proc [dbo].[EPA_sys_SetupAppraisalArea]       -- 'Retrieve','mif','20172018','0529','00010452','Appraisal1','ntp'  
	@Operate		varchar(30),
	@UserID			varchar(30), 
	@IDs			varchar(10) = null,
...
set nocount on 
 begin
	...
	if @Operate = 'List' 
	     ...
	     from [dbo].[EPA_sys_appraisalArea] 
		 order by RowNo
    else if @Operate ='Get'
	     select IDs, Code,[Name], Comments, Active,  
			  '<a href="#" title="click on button to delete this record of ' + [Name] + ' " >'  +  @imgA + '</a>'  as [Action], 
		 ...
		 Where IDs = @IDs
		  ...
		  begin try
			   begin tran
			       if exists(select * from [dbo].[EPA_sys_appraisalArea] where IDs =@IDs)
				      begin
						if @Operate ='Delete'
							delete  [dbo].[EPA_sys_appraisalArea]  where    IDs = @IDs  
						else if @Operate ='Update'
 							 update [dbo].[EPA_sys_appraisalArea] 
						...
						insert into   [dbo].[EPA_sys_appraisalArea]

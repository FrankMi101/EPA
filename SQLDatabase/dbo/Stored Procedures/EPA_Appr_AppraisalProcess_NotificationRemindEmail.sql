...
CREATE proc [dbo].[EPA_Appr_AppraisalProcess_NotificationRemindEmail]
...
begin
	...
	declare   @mailBCC varchar(100), @FromAdres varchar(50) ,@header varchar(100), @footer varchar(20), @NoticeType varchar(50)
	declare @mailSubject varchar(200), @mailBody varchar(4000) , @PrincipalID varchar(10), @OwnerID varchar(10)

	set @NoticeType = Case @Result	when 'Not Start' then 'AppraisalStart'
									when 'Incomplete'	then 'NeedCompleteAppraisal'
									when 'SignOff Required'	then 'NeedSignOff'
									else 'Generic'	end


	if exists(select * from [dbo].[EPA_sys_EmailNotificationTemplate] where category = @ApprType and NoticeFor = 'Remind' and NoticeType = @NoticeType  and NoticeApprArea = @ApprType and NoticeGo = @ToWho and NoticeFrom ='AppAdmin' and Active ='X'  )
		select top 1 @mailSubject = [Subject], @mailBody = Template
		from [dbo].[EPA_sys_EmailNotificationTemplate]
		where category = @ApprType and NoticeFor = 'Remind' and NoticeType = @NoticeType  and NoticeApprArea = @ApprType
			and NoticeGo = @ToWho and NoticeFrom ='AppAdmin' and Active ='X'  
	else
		select top 1 @mailSubject = [Subject], @mailBody = Template
		from [dbo].[EPA_sys_EmailNotificationTemplate]
		where category = 'EPA' and NoticeFor = 'Remind' and NoticeType = @NoticeType  and NoticeApprArea = @ApprType
			 and NoticeGo = @ToWho and NoticeFrom ='AppAdmin' and Active ='X'  
	...
	values('Reminder', @SchoolYear,@SchoolCode,@EmployeeID, @Result, 'ALP','Appraiser', getdate(), @DueDate,  @mailSubject,@mailBody,@UserID, app_name(),getdate())

...
-- Drop proc dbo.EPA_sys_MenusTarget
CREATE Procedure [dbo].[EPA_sys_MenusTarget]
 	...
 	if @Value is null
		...
		if exists (select * from dbo.EPA_sys_FunctionMenus 	where TitleID = @TitleID and Appraisal_Code = @appCode)
			...
			insert into   dbo.EPA_sys_FunctionMenus
					(CategoryID, GroupID,ItemID,TitleID,Appraisal_Code,TargetID,lu_User,lu_Function, lu_Date)

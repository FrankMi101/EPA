...
-- ================================================================================== 
--dbo.EPA_Appr_Report_ProfessionalDialog  'Report', 'mif','20172018','0529','00012934', 'Appraisal1','PPA'
CREATE procedure [dbo].[EPA_Appr_Report_ProfessionalDialog] 
	@Operate		varchar(30),
	@UserID			varchar(30),
...
-- from tcdsb_TPA_School_Staff where UserID = @TeacherID and school_year = @SchoolYear
  declare @AppraiseeName varchar(50), @AppraiseeRole varchar(20), @AppraiserRole varchar(20) ,@AchievingGoals varchar(2000), @Development varchar(2000)
  ...
  set @AchievingGoals =  dbo.EPA_getAppraisalComments(@SchoolYear,@SchoolCode,@EmployeeID,@SessionID,'APM22')  
  set @Development    =  dbo.EPA_getAppraisalComments(@SchoolYear,@SchoolCode,@EmployeeID,@SessionID,'APM33')  
   

select dbo.DateF(Appraisal_Date,'YYYYMMDD') Appraisal_Date,  Appraisal_Note , @AchievingGoals as AchievingGoals, @Development as Development,
...
from  [dbo].[EPA_Appr_Comments_APMeeting] as A
left join  dbo.EPA_getReportSignatureRate(@SchoolYear, @SchoolCode,@EmployeeID,@SessionID,'APP95')   as B on A.EmployeeID != ''
where A.School_Year =@SchoolYear and A.School_Code =@SchoolCode and A.EmployeeID =  @EmployeeID and SessionID =@SessionID  and left(ItemCode,5) ='APM11'
order by ItemCode 

...
CREATE Procedure [dbo].[EPA_sys_DashboardALP]
...
BEGIN   
  set nocount on
        select ALP_Result as ALPStatus ,1 as  TeacherName
        from  dbo.EPA_Appraisal_StaffListView  
--		where AppraisalPhase not in ('NTP','LTO') and school_year = '20172018' and school_code = '0529'   
 		where AppraisalPhase not in ('NTP','LTO') and school_year = @SchoolYear and school_code = @SchoolCode   
--		group by ALP_Result

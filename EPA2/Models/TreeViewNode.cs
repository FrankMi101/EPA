...
namespace EPA2
{
    public class TreeViewNode : System.Web.UI.Page
    {
        ...
        public static void BuildingTree(ref TreeView treeView1, List<AppraisalDefinition> myList, string category, string pageSource)
        {
            ...
            try
            {
                ...
                foreach (var item in myList)
                {
                    ...
                    TreeNode myNode = GetLevelNode(item.AppraisalCode, item.AppraisalText, item.AppraisalStatus, myLevel);
                    if (myLevel == "1")
                    {
                        treeView1.Nodes[0].ChildNodes.Add(myNode);
                        n1++;

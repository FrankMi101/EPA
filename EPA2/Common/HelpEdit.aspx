<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="HelpEdit.aspx.cs" Inherits="EPA2.Common.HelpEdit" %>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $(".BottonTab").click(function (event) {
            ...
            if (currentTab != undefined) { $("#lbl" + currentTab).removeClass("selectedTab"); }
            cTab = $(this)[0].id;
        ...
        $("#TextTitleEdit").change(function (event) {
            var value = $("#TextTitleEdit").val()
            SaveContent("Title", value);
        });
        $("#TextSubTitleEdit").change(function (event) {
            var value = $("#TextSubTitleEdit").val()
            SaveContent("SubTitle", value);
        });
        $("#TextHelpEditL").change(function (event) {
            var value = $("#TextHelpEditL").val();
            var type = $("#hfSelectedTab").val();
            SaveContent(type, value);
        });
        ...
        $("#ButtonSave").click(function (event) {
            ...
            //            var helptext = EPA2.Models.WebService1.SaveMessageContent2("Read", UserID, CategoryID, AreaID, ItemCode, userRole, value, onSuccess4, onFailure);
            //        }
            //        else {
    ...
    function SaveContent(type, value) {
        BasePara.Value = value;
        BasePara.ContenType = type;
        var helpte = EPA2.Models.WebService1.SaveHelpContent1("Save", type, BasePara,onSuccess4, onFailure);
    }
    function LoadingHelpAndTitleContent() {
        var title1 = EPA2.Models.WebService1.GetHelpContent1("Read", "Title", BasePara, onSuccess21, onFailure);
        var title2 = EPA2.Models.WebService1.GetHelpContent1("Read", "SubTitle", BasePara, onSuccess22, onFailure);
        var helpte = EPA2.Models.WebService1.GetHelpContent1("Read", "Help", BasePara, onSuccess23, onFailure);
    }
    function LoadingContent() {
        var type = $("#hfSelectedTab").val(); // Help, EP, Message
        BasePara.ContenType = type;
        var helpte = EPA2.Models.WebService1.GetHelpContent1("Read",type,BasePara,  onSuccess23, onFailure);

        //if (vType == "Message") {
        //    if (ItemCode == "Message") {
        //        var userRole = $("#hfUserRole").val();
        //        var helptext = EPA2.Models.WebService1.GetMessageContent2("Read", UserID, CategoryID, AreaID, ItemCode, userRole, onSuccess21, onFailure);
        //    }
        //    else {

<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ApplicationUsers.aspx.cs" Inherits="EPA2.EPAsystem.ApplicationUsers" %>
...
<body>
    <form id="form2" runat="server">
        ...
        <section>
                        ...
                        <asp:GridView ID="GridView1" runat="server" CellPadding="1" Height="100%" Width="100%" GridLines="Both" AutoGenerateColumns="False" BackColor="White" BorderColor="gray" BorderStyle="Ridge" BorderWidth="1px" CellSpacing="1" EmptyDataText="No Appraisal Staff in current search condition" EmptyDataRowStyle-CssClass="emptyData" ShowHeaderWhenEmpty="true">
                            <Columns>
                                ...
                                <asp:TemplateField HeaderText="User Role">
                                    <ItemStyle Width="10%" />
                                    <ItemTemplate>
                                        <asp:TextBox ID="editRole" runat="server" Text='<%# Eval("UserRole") %>' CssClass="myUserRole" Width="100%" Height="100%">  </asp:TextBox>
                                    </ItemTemplate>
                                </asp:TemplateField>
            ...
            <asp:Button ID="btnAddNew" runat="server" Text="Add New Member of " OnClick="btnAddNew_Click" />
            <asp:DropDownList ID="ddlUserRole" runat="server" Height="23px"></asp:DropDownList>
...
<script>
        ...
        $('td > .mySave').click(function (event) {
            try {
                if ($("#hfContentChange").val() == "1") {
                    ...
                    var role = $(this).closest('tr').find('td > .myUserRole').val();
                    ...
                    var result = EPA2.Models.WebService.SaveApplicationUsers("Update", UserID, CategoryID, AreaID, IDs, code, name, role, comm, active, onSuccessUpdate, onFailureUpdate);
    ...
    function DeleteRecord(IDs, roleCode) {
        ...
        if (result) {
            var result = EPA2.Models.WebService.SaveApplicationUsers("Delete", UserID, CategoryID, AreaID, IDs, roleCode, "", "", "", "0", onSuccessDel, onFailureDel);
        }
        else {

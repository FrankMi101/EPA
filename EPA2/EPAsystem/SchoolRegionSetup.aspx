<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="SchoolRegionSetup.aspx.cs" Inherits="EPA2.EPAsystem.SchoolRegionSetup" %>
...
<body>
    <form id="form2" runat="server">
                ...
                <div id="DivRoot" style="width: 100%; height: 600px;">
                    ...
                    <div style="overflow: scroll; width: 100%; height: 100%" onscroll="OnScrollDiv(this)" id="DivMainContent">
                        <asp:GridView ID="GridView1" runat="server" CellPadding="1" Height="100%" Width="100%" GridLines="Both" AutoGenerateColumns="False" BackColor="White" BorderColor="gray" BorderStyle="Ridge" BorderWidth="1px" CellSpacing="1" 
                            EmptyDataText="No Appraisal Staff in current search condition" EmptyDataRowStyle-CssClass="emptyData" ShowHeaderWhenEmpty="true"
                             >
                            <Columns>
                                ...
                                <asp:TemplateField HeaderText="Offier">
                                    <ItemStyle Width="12%" Wrap="True" />
                                    <ItemTemplate>
                                        <asp:TextBox ID="editOfficer" runat="server" Text='<%# Eval("Officer") %>' CssClass="myOfficer" Width="100%" Height="100%">  </asp:TextBox>
                                    </ItemTemplate>
                                </asp:TemplateField>
        ...
        <div id="DDLDIV" class="bubble epahide">
            ...
            <asp:DropDownList ID="ddlOfficer" runat="server" Height="25px"></asp:DropDownList>
        </div>
...
<script>
    ...
    $(document).ready(function () {
        MakeStaticHeader("GridView1", 550, 850, 25, false);
        ...
        //            var IDs = $(this).find('td > .listIDs').text(); // $(this).closest('tr').find('td.listIDs').text();
        //            var code = $(this).find('td > .myCode').val();
        //            var name = $(this).find('td > .myName').val();
        //            var district = $(this).find('td .myDistrict').val();
        //            var supervisor = $(this).find('td .mySupervisor').val();
        //            var officer = $(this).find('td .myOfficer').val();
        //            var comm = $(this).find('td > .myComment').val();
        //            var check = $(this).find('td > .myCheck');
        //            var active = (check[0].childNodes['0'].checked ? "1" : "0");
        //            var result = EPA2.Models.WebService.SchoolRegion("Update", UserID, CategoryID, AreaID, IDs, code, name, comm, active, district, supervisor, officer, onSuccess, onFailure);
        ...
        $('td > .mySave').click(function (event) {
            try {
                if ($("#hfContentChange").val() == "1") {
                    ...
                    var active = (check[0].childNodes['0'].checked ? "1" : "0");
                    var district = $(this).closest('tr').find('td .myDistrict').val();
                    var supervisor = $(this).closest('tr').find('td .mySupervisor').val();
                    var officer = $(this).closest('tr').find('td .myOfficer').val();
                    var result = EPA2.Models.WebService.SchoolRegion("Update", UserID, CategoryID, AreaID, IDs, code, name, comm, active, district, supervisor, officer, onSuccessUpdate, onFailureUpdate);
                    $("#hfContentChange").val("0");
                }
            }
            catch (ex)
        ...
        $('td > .myDistrict').click(function (event) {
        //    IDs = $(this).closest('tr').find('td .listIDs').text();
            eventCell = $(this);
            workingCell = $(this);
            $("#ddlSupervisor").hide();
            $("#ddlOfficer").hide();
            $("#ddlDistrictList").show();
            openDDL(event);
        });
        $('td > .mySupervisor').click(function (event) {
            //    IDs = $(this).closest('tr').find('td .listIDs').text();
            eventCell = $(this);
            workingCell = $(this);
            $("#ddlSupervisor").show();
            $("#ddlOfficer").hide();
            $("#ddlDistrictList").hide();
            openDDL(event);
        });
        $('td > .myOfficer').click(function (event) {
            //    IDs = $(this).closest('tr').find('td .listIDs').text();
            eventCell = $(this);
            workingCell = $(this);
            $("#ddlSupervisor").hide();
        ...
        $("#ddlDistrictList").change(function (event) {
            var selValue = event.target.value;
            workingCell.val(selValue)
            $("#hfContentChange").val("1");
            $("#DDLDIV").fadeToggle("fast");
        });
        $("#ddlSupervisor").change(function (event) {
            var selValue = event.target.value;
            workingCell.val(selValue)
            $("#hfContentChange").val("1");
            $("#DDLDIV").fadeToggle("fast");
        });
        $("#ddlOfficer").change(function (event) {
            var selValue = event.target.value;
            workingCell.val(selValue)
            $("#hfContentChange").val("1");
            $("#DDLDIV").fadeToggle("fast");
    ...
    function DeleteRecord(IDs, roleCode) {
        ...
        if (result) {
            var result = EPA2.Models.WebService.SchoolRegion("Delete", UserID, CategoryID, AreaID, IDs, roleCode, "", "", "0", "", "", "", onSuccessDel, onFailureDel);
        }
        else {
